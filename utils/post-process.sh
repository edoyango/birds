...
# 1. find best videos
bestvids=$(~/birds/scripts/find-best-vids.py --csv "$output_dir/meta.csv" --num-videos 4 --duration 10)
...
do
	...
	ffmpeg -y -ss "$starttime" -t 10 -i "$f2g" \
...
# 4. optimize gif
gifsicle -O3 --lossy=35 -i /tmp/stacked.gif -o "$output_dir/sample.gif"
