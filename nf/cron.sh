...
export SINGULARITY_TMPDIR=$APPTAINER_TMPDIR SINGULARITY_CACHEDIR=$APPTAINER_CACHEDIR
 ...
 GMAIL_APP_PWD=bpcshpyjugjpmbvy nextflow run main.nf --date $date -profile wehi --model_detect ../models/yolov11-birbs.engine --outdir test-output --email_list ../email-lists.csv -process.errorStrategy retry -resume --nsamples 4 --conf 0.62 --imgsz 704
