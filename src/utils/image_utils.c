...
static int convert_image_cpu(image_buffer_t *src, image_buffer_t *dst, image_rect_t *src_box, image_rect_t *dst_box, char color, int verbose) {
    int ret;
...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color, int verbose)
{
    int ret = 0;
    ...
    if (drect.width != dstWidth || drect.height != dstHeight) {
        ...
        p_imcolor[2] = color;
        p_imcolor[3] = color;
        if (verbose) printf("fill dst image (x y w h)=(%d %d %d %d) with color=0x%x\n",
...
int convert_image(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color, int verbose)
{
    int ret;
    ...
    if(src_img->width % 16 == 0 && dst_img->width % 16 == 0) {
        ret = convert_image_rga(src_img, dst_img, src_box, dst_box, color, verbose);
        if (ret != 0) {
            if (verbose) printf("try convert image use cpu\n");
            ret = convert_image_cpu(src_img, dst_img, src_box, dst_box, color, verbose);
        }
    } else {
        if (verbose) printf("src width is not 4/16-aligned, convert image use cpu\n");
        ret = convert_image_cpu(src_img, dst_img, src_box, dst_box, color, verbose);
    }
#endif
    return ret;
...
int convert_image_with_letterbox(image_buffer_t* src_image, image_buffer_t* dst_image, letterbox_t* letterbox, char color, int verbose)
{
    int ret = 0;
    ...
    if (verbose) printf("scale=%f dst_box=(%d %d %d %d) allow_slight_change=%d _left_offset=%d _top_offset=%d padding_w=%d padding_h=%d\n",
    ...
    ret = convert_image(src_image, dst_image, &src_box, &dst_box, color, verbose);
    return ret;
}