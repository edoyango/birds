...
def stack_images(image_paths, output_filename, lower):
    ...
    res = new_image.size
    resized_image = new_image.resize((int(res[0]/lower), int(res[1]/lower)), Image.LANCZOS)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l", help="Reduce resolution by the given factor. E.g., 2 will half the resolution.", default=2)

    args = parser.parse_args()

    input_path = args.input_path
    
    all_image_files = [os.path.join(input_path, file) for file in os.listdir(input_path) if file.endswith(('.jpg', '.png'))]  # Taking up to 10 images
